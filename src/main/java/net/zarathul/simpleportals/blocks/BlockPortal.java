...
public class BlockPortal extends BlockBreakable {
	public static final PropertyEnum<EnumFacing.Axis> AXIS = PropertyEnum.<EnumFacing.Axis>create("axis",
			EnumFacing.Axis.class, new EnumFacing.Axis[] { EnumFacing.Axis.X, EnumFacing.Axis.Y, EnumFacing.Axis.Z });
	...
	public void onEntityCollidedWithBlock(World world, BlockPos pos, IBlockState state, Entity entity) {
		if (!world.isRemote && !entity.isRiding() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.isDead) {
			...
			if (entityCooldowns.containsKey(entity.getUniqueID())) {
				if (world.getTotalWorldTime() - entityCooldowns.get(entity.getUniqueID()) < COOLDOWN)
					return;
				...
				if (world.getTotalWorldTime() - lastCooldownUpdate >= COOLDOWN_UPDATE_INTERVAL) {
					if (entityCooldowns.size() > 0) {
						...
						for (UUID id : expiredCooldowns) {
							entityCooldowns.remove(id);
						}
					}
			...
			if (entity instanceof EntityItem && Config.powerCost > 0 && Config.powerCapacity > 0) {
				...
				if (PortalRegistry.getPower(start) < Config.powerCapacity
						&& OreDictionary.containsMatch(false, Config.powerSources, item)) {
			...
			boolean bypassPowerCost = (entity instanceof EntityPlayerMP
					&& ((EntityPlayerMP) entity).capabilities.isCreativeMode);

			// Check if portal has enough power for a port
			if (!bypassPowerCost && PortalRegistry.getPower(start) < Config.powerCost)
				return;
			...
			if (destinations.size() > 0 && (bypassPowerCost || Config.powerCost == 0
					|| PortalRegistry.removePower(start, Config.powerCost))) {
				WorldServer server;
				...
				if (portTarget != null) {
					...
					EnumFacing entityFacing = (destination.getAxis() == Axis.Y) ? entity.getHorizontalFacing()
							: (destination.getAxis() == Axis.Z)
									? (portTarget.getZ() > destination.getCorner1().getPos().getZ()) ? EnumFacing.SOUTH
											: EnumFacing.NORTH
									: (portTarget.getX() > destination.getCorner1().getPos().getX()) ? EnumFacing.EAST
											: EnumFacing.WEST;
	...
	public void randomDisplayTick(IBlockState state, World world, BlockPos pos, Random rand) {
		if (Config.ambientSoundEnabled && rand.nextInt(100) == 0) {
			world.playSound(pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D, SoundEvents.BLOCK_PORTAL_AMBIENT,
