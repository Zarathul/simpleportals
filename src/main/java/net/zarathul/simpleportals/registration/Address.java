...
public class Address implements INBTSerializable<NBTTagCompound> {
	...
	/**
	 * Gets the number of times the specified blockId is contained in this address.
	 * 
	 * @param blockId
	 *            The blockId.
	 * @return A value between <code>0</code> and <code>4</code>.
	 */
	public int getBlockCount(String blockId) {
		if (!Strings.isNullOrEmpty(blockId)) {
			if (blockCounts.containsKey(blockId))
				return blockCounts.get(blockId);
		}
	...
	public boolean equals(Object obj) {
		...
		for (Entry<String, Integer> blockCount : blockCounts.entrySet()) {
			if (blockCount.getValue() != other.getBlockCount(blockCount.getKey()))
				return false;
		}
	...
	/**
	 * Counts the specified block ids and fills the block count map accordingly.
	 * Also generates the readable name.
	 * 
	 * @param blockIds
	 ...
	 * 
	 * @return A string of the format <code>blockIdxblockCount</code> for every
