...
/**
 * Provides Waila with tooltip information for portals when looking at portal
 * frames.
 */
public final class BlockPortalFrameDataProvider implements IWailaDataProvider {
	...
	public List<String> getWailaBody(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
			IWailaConfigHandler config) {
		if (config.getConfig(WailaRegistry.WAILA_POWER_CAPACITY_KEY)
				|| config.getConfig(WailaRegistry.WAILA_ADDRESS_KEY)) {
			...
			if (config.getConfig(WailaRegistry.WAILA_POWER_CAPACITY_KEY) && Config.powerCost > 0) {
				...
				for (Portal portal : portals) {
					...
					percentage = (Config.powerCapacity > 0)
							? MathHelper.clamp((int) ((long) power * 100 / Config.powerCapacity), 0, 100)
							: 100;

					currenttip.add(I18n.translateToLocalFormatted(WailaRegistry.WAILA_TOOLTIP_POWER_CAPACITY, power,
							Config.powerCapacity, percentage));
				}
			}
			...
			if (config.getConfig(WailaRegistry.WAILA_ADDRESS_KEY)) {
				...
				for (Portal portal : portals) {
					...
					for (String component : addressComponents) {
						currenttip.add(I18n.translateToLocalFormatted(WailaRegistry.WAILA_TOOLTIP_ADDRESS_COMPONENT,
								component.trim()));
					}
				}
			}
